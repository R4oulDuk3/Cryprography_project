...
from src.pgp.util.util import validate_if_algorithm_symmetric_encryption, validate_if_algorithm_asymmetric_encryption
...
class Sender:
    ...
    def prepare_message(self,
        ...
        receiver_public_key: PublicKey = self.key_manager.get_public_key_by_user_email(email=receiver_mail)
        ...
        signing_key = self.key_manager.get_key_pair_by_user_mail(email=sender_mail, password=password).get_private_key()
...
def test_message_send():
    ...
    sender = Sender(key_manager=KeyManager(user_name="user1"),
                    ...
                    session_key_generator=SessionKeyGenerator(),
                    key_serializer=KeySerializer())
    message = sender.prepare_message(message="Hello World",
